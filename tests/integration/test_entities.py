...
class TestEntityCRUD:
    ...
    def test_read_entity_by_id(self):
        ...
        resp = requests.get(f"{BASE}/api/persons/{created['id']}", timeout=10)
        assert resp.status_code == 200
        assert resp.json()["id"] == created["id"]

    def test_read_entity_not_found(self):
        resp = requests.get(f"{BASE}/api/persons/{uuid.uuid4()}", timeout=10)
        assert resp.status_code == 404

    def test_read_entity_invalid_uuid(self):
        resp = requests.get(f"{BASE}/api/persons/not-a-uuid", timeout=10)
        assert resp.status_code == 422

    def test_list_entities_returns_paginated_envelope(self):
        _create_entity("event", "Listed Event")
        resp = requests.get(f"{BASE}/api/events", timeout=10)
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_update_entity(self):
        ...
        resp = requests.put(
            f"{BASE}/api/persons/{created['id']}", json=update_payload, timeout=10
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_delete_entity(self):
        ...
        del_resp = requests.delete(f"{BASE}/api/persons/{created['id']}", timeout=10)
        assert del_resp.status_code == 200
        assert "deleted" in del_resp.json()["message"].lower()

        # Confirm it's gone
        get_resp = requests.get(f"{BASE}/api/persons/{created['id']}", timeout=10)
        assert get_resp.status_code == 404

    def test_delete_entity_not_found(self):
        resp = requests.delete(f"{BASE}/api/persons/{uuid.uuid4()}", timeout=10)
        assert resp.status_code == 404
    ...
    def test_delete_entity_cascades_connections(self):
        ...
        requests.delete(f"{BASE}/api/places/{place['id']}", timeout=10)
