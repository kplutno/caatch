...
class TestConnectionCRUD:
    def test_create_connection_lives_in(self):
        person = _create_entity("person", "Person-LI")
        place = _create_entity("place", "Place-LI")
        conn = _create_connection(person["id"], place["id"], "LIVES_IN")
        assert conn["label"] == "LIVES_IN"
        assert conn["source_id"] == person["id"]
        assert conn["target_id"] == place["id"]
        assert "id" in conn

    def test_create_connection_knows(self):
        p1 = _create_entity("person", "KnowsA")
        p2 = _create_entity("person", "KnowsB")
        conn = _create_connection(p1["id"], p2["id"], "KNOWS")
        assert conn["label"] == "KNOWS"

    def test_create_connection_works_with(self):
        p1 = _create_entity("person", "WorksA")
        p2 = _create_entity("person", "WorksB")
        conn = _create_connection(p1["id"], p2["id"], "WORKS_WITH")
        assert conn["label"] == "WORKS_WITH"
    ...
    def test_create_connection_participated_in(self):
        person = _create_entity("person", "Participant")
        event = _create_entity("event", "Marathon")
        conn = _create_connection(person["id"], event["id"], "PARTICIPATED_IN")
        assert conn["label"] == "PARTICIPATED_IN"
    ...
    def test_create_connection_with_description(self):
        ...
        conn = _create_connection(
            person["id"], place["id"], "LIVES_IN", description="Moved here in 2022"
        )
    ...
    def test_create_connection_invalid_target_type(self):
        """person LIVES_IN person is disallowed (target must be a place)."""
        p1 = _create_entity("person", "Src Person")
        p2 = _create_entity("person", "Target Person")
        resp = requests.post(
            f"{BASE}/api/connections",
            json={"source_id": p1["id"], "target_id": p2["id"], "label": "LIVES_IN"},
            timeout=10,
        )
        assert resp.status_code == 400
        assert "not allowed" in resp.json()["detail"]
...
class TestConnectionRules:
    ...
    def test_rules_knows_only_targets_person(self):
        resp = requests.get(f"{BASE}/api/connections/rules", timeout=10)
        targets = resp.json()["person"]["KNOWS"]
        assert targets == ["person"]
